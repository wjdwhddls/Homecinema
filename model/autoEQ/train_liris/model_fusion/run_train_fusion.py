"""CLI — Phase 2a-5 Fusion variant training.

Reuses BASE's `PrecomputedLirisDataset` and `liris_panns_v5spec/features.pt`
— no precompute step. Only the fusion stage changes per `--fusion-mode`.
"""
...
from .model import AutoEQModelLirisFusion
...
def override_from_args(
    cfg: TrainLirisConfigFusion, args: argparse.Namespace
) -> TrainLirisConfigFusion:
...
def main() -> int:
    ...
    p.add_argument("--fusion-mode", choices=["gate", "concat", "gmu", "gmu_notanh"], required=True)
    ...
    if args.run_name is None:
        cfg.run_name = f"2a5_{cfg.fusion_mode}_s{cfg.seed}"
    if args.output_dir is None:
        cfg.output_dir = f"runs/phase2a/{cfg.run_name}"
    set_seed(cfg.seed)
    device = cfg.resolved_device()
    print(f"[run] device={device} fusion_mode={cfg.fusion_mode}")
    ...
    model = AutoEQModelLirisFusion(cfg).to(device)
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[model] trainable params: {n_trainable/1e6:.2f}M (fusion_mode={cfg.fusion_mode})")
