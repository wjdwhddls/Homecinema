"""CLI — Phase 2a-4 CLIP frame-mean variant training.

Mirrors `model/autoEQ/train_liris/run_train.py` but swaps:
    TrainLirisConfig           → TrainLirisConfigCLIPMean
    AutoEQModelLiris           → AutoEQModelLirisCLIPMean
    PrecomputedLirisDataset    → PrecomputedLirisDatasetCLIPMean
...
axis under ablation is the visual encoder / feature path (V5-FINAL §21 OAT).
"""
...
from .model import AutoEQModelLirisCLIPMean
...
def build_loaders(
    cfg: TrainLirisConfigCLIPMean,
    features: dict,
    splits: dict[str, pd.DataFrame],
) -> dict[str, DataLoader]:
    ds_train = PrecomputedLirisDatasetCLIPMean(splits["train"], features)
    ds_val = PrecomputedLirisDatasetCLIPMean(splits["val"], features)
    ds_test = PrecomputedLirisDatasetCLIPMean(splits["test"], features)
...
def override_from_args(
    cfg: TrainLirisConfigCLIPMean, args: argparse.Namespace
) -> TrainLirisConfigCLIPMean:
...
def main() -> int:
    ...
    p.add_argument("--run-name", default="2a4_clipmean")
    ...
    p.add_argument("--clip-model-name", type=str, default=None)
    args = p.parse_args()
    ...
    model = AutoEQModelLirisCLIPMean(cfg).to(device)
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[model] trainable params: {n_trainable/1e6:.2f}M (CLIPMean variant, visual_dim={cfg.visual_dim})")
