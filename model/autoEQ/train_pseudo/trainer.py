...
class TrainerCog:
    ...
    def train_one_epoch(self) -> dict:
        ...
        gate_w_v_sum = 0.0
        gate_w_a_sum = 0.0
        gate_w_v_sq_sum = 0.0  # for variance
        gate_w_a_sq_sum = 0.0
        gate_entropy_sum = 0.0  # raw entropy value (not loss term)
        num_batches = 0
        ...
        for batch in self.train_loader:
            ...
            gate_w_v_sq_sum += (gw[:, 0] ** 2).mean().item()
            gate_w_a_sq_sum += (gw[:, 1] ** 2).mean().item()
            # raw entropy: -sum(w log w). Positive; max = ln(2) ≈ 0.693
            eps = 1e-8
            ent = -(gw * torch.log(gw + eps)).sum(dim=-1).mean().item()
            gate_entropy_sum += ent
            num_batches += 1
        ...
        if num_batches > 0:
            ...
            # sample variance of per-batch means (proxy for gate instability)
            mean_v = gate_w_v_sum / num_batches
            mean_a = gate_w_a_sum / num_batches
            total_losses["gate_w_v_var"] = max(0.0, gate_w_v_sq_sum / num_batches - mean_v ** 2)
            total_losses["gate_w_a_var"] = max(0.0, gate_w_a_sq_sum / num_batches - mean_a ** 2)
            total_losses["gate_entropy_value"] = gate_entropy_sum / num_batches
