...
def load_frames_from_mp4(
    ...
    # Read window spanning the frames: [min(ts), max(ts)+small_pad]
    t_lo = max(0.0, float(min(timestamps)) - 0.05)
    t_hi = float(max(timestamps)) + 0.05
    video, _, info = read_video(
        str(video_path), start_pts=t_lo, end_pts=t_hi, pts_unit="sec"
    )
    # video: (T, H, W, C) uint8
    ...
    for ts in timestamps:
        idx = int(round((ts - t_first) * fps))
        idx = max(0, min(video.shape[0] - 1, idx))
        frame = video[idx].permute(2, 0, 1).float() / 255.0  # (C, H, W)
