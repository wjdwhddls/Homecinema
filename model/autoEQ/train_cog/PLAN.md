# train_cog: CogniMuse 전용 학습 파이프라인 구현 계획

## Context

원 계획은 `model/autoEQ/train/`(LIRIS-ACCEDE 160편/9,800클립 기반)에서 학습, CogniMuse(7편/3.5h 연속 주석)를 OOD 검증에 쓰는 것이었음. LIRIS-ACCEDE 확보 불가로 CogniMuse만으로 학습·검증 모두 수행해야 하나, 기존 `train/`은 LIRIS 복구 대비 **그대로 보존**하고 `train_cog/`를 신규 생성해 CogniMuse 전용 파이프라인을 구축한다.
...
12. **재사용 전략: Import 공유** — train_cog는 train/의 frozen 자산만 import (`encoders.py`, `utils.py` 일부, `precompute.py`의 tensor 함수, `dataset.py`의 `MOOD_CENTERS`/`va_to_mood`)
...
15. **Plan 문서 위치: `train_cog/PLAN.md`**. 기존 `train/COGNIMUSE_TRANSITION_PLAN.md`는 historical reference로 사용자가 수동 정리
...
| `MOOD_CENTERS`, `va_to_mood` | `from ..train.dataset import ...` | K=7 기본. K=4 축소 시 train_cog 내부 `MOOD_CENTERS_4Q` 정의 |
...
### 1. `train_cog/config.py` — TrainCogConfig (독립)
...
### 2. `train_cog/model.py` — AutoEQModelCog
...
### 5. `train_cog/trainer.py` — TrainerCog
...
### 6. `train_cog/cognimuse_preprocess.py` — 독립 I/O
...
### 7. `train_cog/analyze_cognimuse_distribution.py` — Phase 0 게이트
...
### 8. `train_cog/run_train.py` — 단일 fold 학습
...
### 9. `train_cog/run_lomo.py` — 7-fold orchestrator
...
| 순 | 대상 | 비고 |
|---|---|---|
| 1 | `train_cog/__init__.py`, `config.py` | 최소 seed |
| 2 | `train_cog/model.py` | cong 없는 구조 |
| 3 | `train_cog/losses.py` | 3-term |
| 4 | `train/utils.py`에 RMSE 필드 추가 | 기존 무수정 원칙의 유일한 예외 |
| 5 | `train_cog/dataset.py` | LOMO 스플리터 + apply_sigma_filter |
| 6 | `train_cog/trainer.py` | CCC+MAE tiebreaker |
| 7 | `train_cog/cognimuse_preprocess.py` | I/O 독립 구현 |
| **8** | **CogniMuse 원본 파일 수령 → 구조 assumption 검증** | **여기서 실제 파일 구조 확인하고 plan assumption 블록 수정** |
| 9 | `train_cog/analyze_cognimuse_distribution.py` + **Phase 0 실제 실행 (게이트)** | K=7/4 확정, `lambda_mood` 확정, `MOOD_CENTERS_4Q` 사용 여부 확정 |
| 10 | `train_cog/run_train.py`, `run_lomo.py` (Phase 0 결과 반영 default) | fold_mapping.json 저장 |
| 11 | `train_cog/tests/` 일괄 작성 | 8개 파일 |
...
- K=7 유지 → 이후 수정 없음
- `lambda_mood` 하향 → `train_cog/config.py` 1곳
- K=4 축소 → `train_cog/config.py` `num_mood_classes=4` + `train_cog/dataset.py`에서 `MOOD_CENTERS_4Q` 활성 + `test_cog_model_shapes.py` K 수정

## 학습 프로세스 다이어그램

```
CogniMuse 원본(7편 MP4 + 40ms V/A .txt, experienced)
    ↓ train_cog/cognimuse_preprocess.py (독립 I/O)
...
data/features/cognimuse/{cognimuse_visual.pt, _audio.pt, _metadata.pt}
    ↓ train_cog/analyze_cognimuse_distribution.py (Phase 0 게이트)
    ↓
...
### 0. Phase 0 게이트
```bash
python -m model.autoEQ.train_cog.cognimuse_preprocess \
    --cognimuse_dir /path/to/cognimuse \
    --output_dir data/features/cognimuse \
    --annotation experienced

python -m model.autoEQ.train_cog.analyze_cognimuse_distribution \
    --feature_dir data/features/cognimuse \
    --split_name cognimuse \
...
### 1. 단위 테스트
```bash
pytest model/autoEQ/train_cog/tests -x
```
...
### 3. 7-fold LOMO
```bash
python -m model.autoEQ.train_cog.run_lomo \
